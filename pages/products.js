...
export default function Products() {
	...
	const { data: totalProducts } = useSWR(
		`/products/count?${countQueryString}`,
		fetcher
	);
	...
	const handlePageClick = (selectedPage) => {
		router
			.push(
				`/products?page=${selectedPage}${
					categoryQueryValue ? `&category=${categoryQueryValue}` : ""
				}${searchQueryValue ? `&search=${searchQueryValue}` : ""}`,
				undefined,
				{ shallow: true }
			)
	...
	const renderProducts = !products ? (
		...
		<div className="contentColumn">
			...
			{products.length === 0 && (
				<p className="noResultsFound">No products found</p>
			)}
	...
	return (
		<div className={styles.container}>
			<Head>
				...
				<meta
					name="title"
					content="Products | Halfmoon Manga + Anime"
				/>
				<meta
					property="og:title"
					content="Products | Halfmoon Manga + Anime"
				...
				<meta
					property="og:url"
					content="https://animanga.me/products"
				/>
				<meta
				...
				<meta
					...
					content="Products | Halfmoon Anime & Manga"
				/>
			...
			<main className="main">
				...
				<p className={styles.productsMessage}>
					All products are subject to store availability. If you would
					like to reserve an item, please give us a call at{" "}
					<Link href="tel:123-456-7890">
						<a>(604) 301-9075</a>
					</Link>{" "}
					and we'd be happy to put the item aside for you!
				</p>
